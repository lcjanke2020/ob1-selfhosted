...
| **DB init smoke test** | A `db/*.sql` change that breaks fresh init, status migration, role-manifest parity, or the least-privilege assertions (e.g. the monitor reaches anything beyond the raw log)              | `scripts/ci/run_db_init_smokes.sh all` runs every corpus family; `scripts/ci/run_log_sink_smokes.sh all` runs the independent sink lifecycle. See [the family command reference](scripts/ci/README.md). The completed-catalog assertions live in [`db/03-grants-assertion.sql`](db/03-grants-assertion.sql) and [`db/log-sink/02-log-sink-assertion.sql`](db/log-sink/02-log-sink-assertion.sql); sink identities and grants are pinned by [`role-contract.json`](db/log-sink/role-contract.json).                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
