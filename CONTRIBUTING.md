...
| **Caddyfile validate** | A Caddyfile that doesn't parse under the pinned Caddy image | `docker run --rm -v $PWD/deploy/compose-tailnet/Caddyfile:/etc/caddy/Caddyfile <pinned caddy image> caddy validate --adapter caddyfile --config /etc/caddy/Caddyfile` |
| **DB init smoke test** | A `db/*.sql` change that breaks fresh init or violates the least-privilege assertions (e.g. the monitor role must never be able to read `thoughts`) | Run the compose-local stack against a fresh volume; assertions live in `db/03-grants-assertion.sql` |
