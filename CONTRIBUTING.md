...
| **DB init smoke test** | A `db/*.sql` change that breaks fresh init or violates the least-privilege assertions (e.g. the monitor role gains access to any relation beyond its two-table allowlist)                 | `scripts/ci/run_db_init_smokes.sh all` starts the throwaway corpus database and runs every extracted family; see [the family command reference](scripts/ci/README.md). The assertions live in [`db/03-grants-assertion.sql`](db/03-grants-assertion.sql), and [`db-init.yml`](.github/workflows/db-init.yml) adds the separate log-sink lifecycle job.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
