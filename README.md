...
- **Bounded MCP recall** — lookup, list, and search results are capped at 120,000 serialized UTF-8 bytes, leaving framing headroom below hosted-connector limits. Results that fit retain their existing shape; an oversized result keeps complete records where possible and reports omitted IDs or fields plus a deterministic `fetch`, `session_lookup`, narrower-query, or tailnet REST recovery path. REST payloads are not subject to this MCP-only budget.
