...
> **Why Tailscale Funnel and not a Cloudflare Tunnel?** Because Cloudflare would terminate your TLS at its edge and see the plaintext of every memory you capture — which breaks the project's one promise, *your data never leaves hardware you own*. The full trade-off, including where Cloudflare **is** the right call, is in [`docs/why-not-cloudflare.md`](docs/why-not-cloudflare.md).
