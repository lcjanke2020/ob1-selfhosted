...
- **The ingress qube keeps one INSERT-only path to the db qube** while the
  log-ingester lives on the edge — a scoped, documented exception whose future
  home is an edge-local logs store
  ([#12](https://github.com/lcjanke2020/ob1-selfhosted/issues/12)).
  ([`three-qube-design.md` § Log-ingester placement](../deploy/qubes/three-qube-design.md#log-ingester-placement-decided-for-now))
