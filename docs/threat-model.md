...
- **Credentials** — interactive OAuth client secrets, M2M service-account secrets held by scheduled agents, the local `x-brain-key`, five Postgres role passwords, optional Pushover/ntfy delivery credentials, and the Tailscale node identity. Blast radii: the two verifiers in [`security-model.md` § Trust boundaries](security-model.md#trust-boundaries), the DB roles in [§ Database layer](security-model.md#database-layer), notification metadata in [§ Audit layer](security-model.md#audit-layer), and the node identity in [`funnel-mcp-perimeter.md`](funnel-mcp-perimeter.md).
- **Audit integrity** — Caddy's access logs, `funnel_access_log`, `mcp_auth_events`, and `metadata_degradation_events`: the evidence trail of what reached the doors and whether classification degraded — request metadata, reason-coded auth failures, content-free classifier events, and per-write door/`sub` attribution. It shows who knocked and what was written, not every read of the store: there is no per-tool or per-row read audit.
...
- **Local compose** — one boundary: possession of the `x-brain-key` plus network reach. Anyone holding both can use shared workspace/project audiences. Personal visibility is disabled unless the operator binds every key holder to one deployment-wide `MCP_ACCESS_KEY_PRINCIPAL`.
...
- [**Database**](security-model.md#database-layer) — five least-privilege roles plus forced RLS on memory rows; missing audience context matches nothing, the app cannot DELETE thoughts, and the ingester can only INSERT into one observability table.
...
- **Thought attribution is last-writer-wins on dedupe.** Re-capturing byte-identical content through another door updates `door`/`sub`; explicit new caller provenance replaces the prior claims object. Those author/agent/repo/branch values are validated assertions, not authenticated identity, and one deduplicated row is not a contributor history. ([`security-model.md` § Known limitations](security-model.md#known-limitations))
...
The documented trust model itself is not a vulnerability: shared-key holders
share one configured principal, and authenticated callers share every
