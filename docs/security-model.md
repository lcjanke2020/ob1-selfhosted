...
- Bearer validation pins issuer, audience, algorithm (RS256), and requires `exp` and `sub` claims; verification fails closed before any source-marker stamping runs. Only after verification does a signed Auth0 `gty=client-credentials` claim—or an exact operator-configured subject for another issuer—select the `service` provenance label. That mapping changes neither authentication nor authorization.
