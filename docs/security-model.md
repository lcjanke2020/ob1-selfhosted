...
- A CI guard (`server/scripts/check_allow_env.ts`) rejects bare Compose `--allow-env` grants and keeps checked-in Dockerfile/Compose Deno launchers in lockstep with reachable env reads. Host-local systemd units are outside repository CI and require the documented pre-restart config probe in [`CONTRIBUTING.md`](../CONTRIBUTING.md#native-deno-launchers).
