...
The system has **two auth doors, chosen per deployment** rather than both at once. They're independently toggleable (`MCP_ACCESS_KEY` enables the static-key door; the three `AUTH0_*` vars enable the OAuth door), and the server refuses to boot with neither configured.
