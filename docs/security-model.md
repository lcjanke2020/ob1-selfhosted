...
- A CI guard (`server/scripts/check_allow_env.ts`) rejects unrestricted or dynamic checked-in Deno launchers and keeps Dockerfile/Compose allowlists in lockstep with statically reachable env reads plus explicit out-of-tree dependency policies. Host-local systemd units are outside repository CI and require the documented real-entrypoint pre-restart probe in [`CONTRIBUTING.md`](../CONTRIBUTING.md#native-deno-launchers).
