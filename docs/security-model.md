...
- A CI guard (`server/scripts/check_allow_env.ts`) rejects unrestricted or dynamic checked-in Deno launchers and keeps Dockerfile/effective Compose-stack allowlists in lockstep with statically reachable env reads plus explicit out-of-tree dependency policies. Compose services inherit audited launcher defaults from their checked-in build Dockerfiles before overrides are applied; unresolved image defaults and shell positional boundaries fail closed. The checked-in Deno import configuration is pinned as part of that audit, while custom config/import-map semantics and Node's `process.env` surface are rejected. Host-local systemd units are outside repository CI and require the documented real-entrypoint pre-restart probe in [`CONTRIBUTING.md`](../CONTRIBUTING.md#native-deno-launchers).
