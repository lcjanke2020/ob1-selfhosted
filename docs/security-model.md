...
`openbrain_ingester` and `openbrain_monitor` are roles on a **separate cluster**:
the ingress qube's local Funnel-log sink, a socket-only Postgres holding two
request-metadata relations and nothing else
...
*inside* Postgres, above where a pre-auth wire-protocol or SCRAM-handshake flaw
