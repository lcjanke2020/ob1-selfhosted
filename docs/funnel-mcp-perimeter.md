...
5. **HTTP 401 + RFC 6750 `WWW-Authenticate` (with `resource_metadata`) on *every* auth failure — missing, invalid, and expired credentials alike.** The missing-credentials 401 is what unblocks claude.ai's OAuth discovery; the invalid/expired-credentials 401 is what triggers token refresh and re-authorization after expiry (answer those with HTTP 200 + a JSON-RPC error body and the connector sits disconnected until a human reauthenticates it). A JSON-RPC error envelope *body* on the 401 is fine — MCP clients that surface application-layer errors can still correlate the request `id`.
