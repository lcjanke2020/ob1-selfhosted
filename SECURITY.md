...
A `.githooks/pre-commit` hook blocks commits that would introduce a tailnet IP, an internal ticket id, or a denylisted hostname into tracked files — a courtesy mirror of the CI identifier scan for anyone running their own deployment from a fork. It is opt-in per clone (`git config core.hooksPath .githooks`) and the real hostname denylist stays local (gitignored). See [`.githooks/README.md`](.githooks/README.md). This is convenience, not a security boundary — the project does not rely on obscurity.
