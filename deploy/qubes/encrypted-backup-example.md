...
pipeline is noticed. `OnFailure=` only sees a run that started and failed; a
run that never started (host was off, timer stamp lost, unit not enabled) is
invisible to it — so also put a freshness check on the *receiving* host that
