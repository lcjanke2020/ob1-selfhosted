...
// initialization (for a Pool, `client.connect()`). When initialization
// throws — PostgreSQL unreachable — the element is never returned to
// the stack: the caller gets a rejection and no handle, so the slot is
// permanently lost. Two failed first borrows against a size-2 lazy pool
// (the dedicated auth-audit and log-ingester pools) consume both slots;
// every later borrow parks forever in the wait queue and the consumer
// hangs even after PostgreSQL recovers. The patch restores the element
// to the stack before rethrowing, so capacity is preserved, parked
// waiters are woken (each receives the element, retries initialization,
// and fails fast in turn while the database is down), and the next
// borrow after recovery reconnects — which is exactly the contract
// db_pool.ts's getClient() was written against.
...
export class DeferredAccessStack<T> {
  ...
  async pop(): Promise<T> {
    ...
    if (!(await this.#checkElementInitialization(element))) {
      // PATCH (PR55-SRV-001): restore the element before rethrowing a
      // failed initialization so the slot is not permanently lost.
      // Upstream awaits `#initializeElement(element)` bare here.
      try {
