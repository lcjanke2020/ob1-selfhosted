...
type OrderedFixture = { branch: string; expected: number[] };
type Fixture = {
  audit: OrderedFixture;
  explicit: OrderedFixture;
  triggered: OrderedFixture;
  tie: OrderedFixture;
};
...
async function seedFixture(): Promise<Fixture> {
  return await withAdmin(async (client) => {
    ...
    // The session_updated_at trigger is the server-owned half of effective
    // freshness. Exercise a real UPDATE rather than assigning updated_at in
    // the fixture, and prove that its fallback value can win the public order.
    const triggeredBranch = `${fixturePrefix}triggered`;
    const triggeredOlderExplicit = await insertFixture(client, {
      title: `${fixturePrefix}triggered-older-explicit`,
      branch: triggeredBranch,
      lastUpdate: "2001-01-01T00:00:00Z",
      updatedAt: "2001-01-01T00:00:00Z",
    });
    const triggeredImplicit = await insertFixture(client, {
      title: `${fixturePrefix}triggered-implicit`,
      branch: triggeredBranch,
      lastUpdate: null,
      updatedAt: "2000-01-01T00:00:00Z",
    });
    const advanced = await client.queryObject<{ advanced: boolean }>(
      `UPDATE sessions.session
       SET title = title
       WHERE id = $1
       RETURNING updated_at > $2::timestamptz AS advanced`,
      [triggeredImplicit, "2001-01-01T00:00:00Z"],
    );
    assertEquals(
      advanced.rows[0].advanced,
      true,
      "session_updated_at did not advance the server-managed timestamp",
    );
    ...
    return {
      ...
      triggered: {
        branch: triggeredBranch,
        expected: [triggeredImplicit, triggeredOlderExplicit],
      },
...
try {
  ...
  await assertLookupAndListAgree(
    "trigger-managed fallback timestamp",
    fixture.triggered.branch,
    fixture.triggered.expected,
  );
  await assertLookupAndListAgree(
  ...
  console.log(
    "session ordering smoke passed null, explicit, trigger, and tie cases",
  );
} finally {
