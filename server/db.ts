...
try {
  ...
  console.log(
    `[db] postgres reachable; hybrid-search + passage-index + fail-closed spaces + metadata audit + native-token + auth-audit + thought-mutation schema ready at ${DB_HOST}:${DB_PORT}`,
  );
} catch (e) {
